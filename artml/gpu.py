...
class GPU():
    ...
    def __compute_BET(self, dataframe=None):
        ...
        input_matrix = cp.array(input_matrix)
        rows, cols = input_matrix.shape
        input_matrix_sq = cp.power(input_matrix, 2)
        input_matrix_transpose = input_matrix.T
        input_matrix_sq_transpose = input_matrix_sq.T

        single_mul_arr = cp.matmul(input_matrix_transpose, input_matrix)
        double_mul_arr = cp.matmul(input_matrix_sq_transpose, input_matrix_sq)

        length_arr = cp.full((cols,1), rows, dtype=self.__precision)
        sum_arr = cp.matmul(input_matrix_transpose, cp.ones((rows,1), dtype=self.__precision))
        double_arr = cp.diag(single_mul_arr).reshape(cols, 1)
        triple_arr = cp.diag(cp.matmul(input_matrix_sq_transpose, input_matrix)).reshape(cols, 1)
        quad_arr = cp.diag(double_mul_arr).reshape(cols, 1)

        stack = cp.dstack(cp.broadcast_arrays(length_arr, sum_arr, double_arr, triple_arr, quad_arr, length_arr.T, sum_arr.T, double_arr.T, triple_arr.T, quad_arr.T, single_mul_arr, double_mul_arr)).tolist()
